...
O NuGet 4.0 e superior pode trabalhar diretamente com as informações em um arquivo `.csproj` sem a necessidade de um arquivo `.nuspec` ou `project.json` separado. Todos os metadados anteriormente armazenados nesses arquivos de configuração podem ser armazenados no arquivo `.csproj` diretamente em vez disso, conforme descrito aqui.
...
- [Ordem de build de destino](#target-build-order)
...
| VersionPrefix | PackageVersionPrefix | empty | A configuração PackageVersion substituirá PackageVersionPrefix |
| VersionSuffix | PackageVersionSuffix | empty | $(VersionSuffix) do MSBuild. A configuração PackageVersion substituirá PackageVersionSuffix | 
...
Se um arquivo do tipo Compilação estiver fora da pasta de projeto, ele é adicionado apenas a `src\<ProjectName>\`.
