...
- [Seções de origem de pacote](#package-source-sections): -[packageSources](#packagesources)
...
Observação: `dependencyVersion` e `repositoryPath` se aplicam apenas a projetos que usam `packages.config`. `globalPackagesFolder` aplica-se somente a projetos que usam `project.json` e formatos de PackageReference.
