...
> Observe que o pacote de símbolos não é a única estratégia para disponibilizar os símbolos de depuração para os consumidores da sua biblioteca. Também é [possível `embed` fazer](/dotnet/core/deploying/single-file#include-pdb-files-inside-the-bundle) isso no `dll` ou `exe` com a seguinte propriedade de projeto:`<DebugType>embedded</DebugType>`
