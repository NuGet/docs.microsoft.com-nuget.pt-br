...
*Para projetos do .NET Core que usam o NuGet 4.0 ou superior, consulte [Empacotamento e restauração do NuGet como destinos do MSBuild](../schema/msbuild-targets.md) para ver detalhes sobre direcionamento cruzado.*
...
- [Estrutura da pasta de versão do Framework](#framework-version-folder-structure)
- [Correspondendo versões de assembly e a estrutura de destino em um projeto](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [Agrupando assemblies por versão de estrutura](#grouping-assemblies-by-framework-version)
- [Determinar qual destino do NuGet deve ser usado](#determining-which-nuget-target-to-use)
- [Arquivos de conteúdo e scripts do PowerShell](#content-files-and-powershell-scripts)
...
Para ver uma lista completa de nomes compatíveis, consulte a [Referência a Estruturas de Destino](../schema/target-frameworks.md#supported-frameworks).
...
> [!Warning]
> Arquivos de conteúdo mutáveis e a execução de script estão disponíveis somente no formato `packages.config`; eles são preteridos ao usar `project.json` e formatos PackagesReference e não deve ser usado para nenhum pacote novo.
