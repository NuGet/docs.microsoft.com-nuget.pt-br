...
O `LOWER_ID` valor é a ID de pacote desejada com letras minúsculas usando as regras implementadas pelo. Método NET [`System.String.ToLowerInvariant()`](/dotnet/api/system.string.tolowerinvariant?view=netstandard-2.0#System_String_ToLowerInvariant) .
...
carteira de licença        | string                     | no       | 
listados                   | booleano                    | no       | Deve ser considerado como listado, se ausente
minClientVersion         | string                     | no       | 
projectUrl               | string                     | no       | 
published                | string                     | no       | Uma cadeia de caracteres que contém um carimbo de data/hora ISO 8601 de quando o pacote foi publicado
requireLicenseAcceptance | booleano                    | no       | 
resumo                  | string                     | no       | 
marcas                     | Cadeia de caracteres ou matriz de cadeia de caracteres  | no       | 
título                    | string                     | no       | 
