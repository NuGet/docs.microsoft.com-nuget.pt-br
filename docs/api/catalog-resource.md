...
Criado                 | cadeia de caracteres                     | no       | Um carimbo de hora de quando o pacote foi criado. Propriedade fallback: `published`.
...
isPrerelease            | boolean                    | no       | Se a versão do pacote está em pré-lançamento. Podem ser detectados de `version`.
