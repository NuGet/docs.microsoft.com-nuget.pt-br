...
Para ver as perguntas frequentes relacionadas ao NuGet.org, como perguntas a respeito da conta do NuGet.org, confira [Perguntas frequentes sobre o NuGet.org](../nuget-org/nuget-org-faq.md).
...
- O Visual Studio Code (todas as plataformas) não tem nenhuma integração direta do NuGet. Use a [CLI do NuGet](../reference/nuget-exe-cli-reference.md) ou a [CLI do dotnet](../reference/dotnet-commands.md).
- O Azure DevOps fornece [uma etapa de build para restaurar os pacotes do NuGet](/vsts/build-release/tasks/package/nuget). Você também pode [hospedar feeds privados de pacote do NuGet no Azure DevOps](/azure/devops/artifacts/nuget/publish).
...
Consulte o [Guia de instalação](../install-nuget-client-tools.md). Para verificar a versão atual instalada da ferramenta, use `nuget help`.
...
**Tento converter a variável $DTE no tipo DTE2, mas recebo um erro: não é possível converter o valor "EnvDTE. DTEClass" do tipo "EnvDTE. DTEClass" no tipo "EnvDTE80. DTE2". Qual é o problema?**
...
Consulte [Criar e publicar um pacote](../quickstart/create-and-publish-a-package-using-visual-studio.md).

**Tenho várias versões da minha biblioteca que visam diferentes versões do .NET Framework. Como fazer criar um único pacote que dá suporte a isso?**
...
Defina a [`repositoryPath`](../reference/nuget-config-file.md#config-section) configuração em `Nuget.Config` usando `nuget config -set repositoryPath=<path>` .
...
Consulte [Habilitar e desabilitar a restauração de pacote](../consume-packages/package-restore.md#enable-and-disable-package-restore-in-visual-studio).
