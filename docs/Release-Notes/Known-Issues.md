...
A causa é que certos arquivos dos pacotes antigo estão bloqueados por um processo do MSBuild em segundo plano.
Mesmo depois de reiniciar o VS, o processo de MSBuild em segundo plano ainda usa os arquivos nos pacotes antigos, causando falhas de build.

A correção é instalar a Atualização do VS 2012, por exemplo, [VS 2012 Atualização 2](http://www.microsoft.com/download/details.aspx?id=38188).
...
Se você tentar desinstalar o NuGet por meio do Gerenciador de Extensões do Visual Studio, poderá perceber que o botão Desinstalar está desabilitado.
O NuGet exige privilégios de administrador para ser instalado e desinstalado. Reinicie o Visual Studio como administrador para desinstalar a extensão.
O NuGet não requer acesso de administrador para usá-lo.
...
Se você tiver instalado o Visual Studio 2010 SP1 Beta, poderá perceber que, se você deixar o Console do Gerenciador de Pacotes abertos e fechar o Visual Studio, ele falhará. Este é um problema conhecido do Visual Studio e será corrigido na versão SP1 RTM.
Por enquanto, basta ignorar a falha ou desinstalar o SP1 Beta, se possível.
...
Se você instalou pacotes compilados com uma versão de pré-lançamento do NuGet, poderá encontrar uma mensagem de erro dizendo “o elemento 'metadata' no namespace 'schemas.microsoft.com/packaging/2010/07/nuspec.xsd' tem um elemento filho inválido” ao executar a versão RTM do NuGet com esse projeto. Você precisará desinstalar e reinstalar cada pacote usando a versão RTM do NuGet.

## <a name="attempting-to-install-or-uninstall-results-in-the-error-cannot-create-a-file-when-that-file-already-existsrdquo"></a>Tentativa de instalar ou desinstalar resultou no erro “Não é possível criar um arquivo quando ele já existe.&rdquo;
...
2. Abra a pasta a seguir (ela pode estar em uma unidade diferente no seu computador)
...
3. Exclua todos os arquivos com a extensão *.deleteme*.
...
Para relatar problemas em clientes do NuGet, acesse [aqui](https://nuget.codeplex.com/WorkItem/Create).
Para relatar problemas na galeria do NuGet, acesse [aqui](https://github.com/nuget/nugetgallery/issues).
