...
Por padrão, `Update-Package` afeta todos os pacotes em uma solução. Para limitar a ação a um projeto específico, use a opção `-ProjectName`, utilizando o nome do projeto como ele aparece no Gerenciador de Soluções:
