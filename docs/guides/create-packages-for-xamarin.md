...
1. No Visual Studio 2017, **arquivo > novo projeto de >**, procure `Plugin` , selecione o modelo de **plug-in de biblioteca .net Standard de plataforma cruzada** , altere o nome para LoggingLibrary e clique em OK.

    ![Novo projeto de aplicativo em branco (aplicativos Xamarin. Forms portátil) no VS 2017](media/CrossPlatform-NewProject.png)

    No Visual Studio 2019, **arquivo > novo projeto de >**, procure `Plugin` , selecione o modelo de **plug-in de biblioteca .net Standard de plataforma cruzada** e clique em Avançar.
...
- O outro projeto compartilhado contém código no `CrossLoggingLibrary.shared.cs` que localizará uma implementação específica da plataforma da interface abstrata em tempo de execução. Normalmente não é necessário modificar esse arquivo.
- Os projetos específicos da plataforma, como `LoggingLibrary.android.cs` , cada um contêm uma implementação nativa da interface em seus arquivos respectivos `LoggingLibraryImplementation.cs` (vs 2017) ou `LoggingLibrary.<PLATFORM>.cs` (vs 2019). É aqui que você compila o código da biblioteca.
...
1. Abra o `LoggingLibraryImplementation.cs` arquivo (vs 2017) ou `LoggingLibrary.<PLATFORM>.cs` (vs 2019) de cada projeto de plataforma e adicione o código necessário. Por exemplo (usando o `Android` projeto de plataforma):
