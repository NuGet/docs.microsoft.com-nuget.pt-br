...
A maneira mais fácil de consumir o catálogo é usar o pacote do SDK do catálogo de pré-lançamento .NET: [NuGet.Protocol.Catalog](https://dotnet.myget.org/feed/nuget-build/package/nuget/NuGet.Protocol.Catalog). Este pacote está disponível no feed `nuget-build` MyGet, para o qual você usa a URL `https://dotnet.myget.org/F/nuget-build/api/v3/index.json` de origem do pacote do NuGet.
